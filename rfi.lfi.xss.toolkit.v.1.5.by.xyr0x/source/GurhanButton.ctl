...
'-Several Styles => gbStandard, gbFlat, gbOfficeXP, gbWinXP, gbNoBorder
...
'
'CREDITS:
'-Some XPWindowsButton ideas and Ampersand Management
' by Gonchuki-gonchuki@yahoo.es
...
''''''    UserControl.ForeColor = PropBag.ReadProperty("ForeColor", &H80000012)
    ...
    m_DefCurHand = PropBag.ReadProperty("DefCurHand", 1)
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("DefCurHand", m_DefCurHand, 1)
...
Private Sub UserControl_Initialize()
    ...
    ScaleMode = 3
    PaletteMode = 3 'vbPaletteModeContainer
End Sub
...
End Sub
Public Sub Refresh()
    AutoRedraw = True 'We're doing this to get rid _
                        of a VB Bug: No AutoRefresh when BitBlt!
    'Clearing everything
    UserControl.Cls
...
Private Sub UserControl_KeyDown(KeyCode As Integer, Shift As Integer)
    If g_KeyPressed = 0 Then ' Not continuous clicking
...
If m_ButtonStyle = gbWinXP Then Exit Sub
    ...
    If m_ButtonStyle = gbOfficeXP Then
        ...
        If g_MouseDown = 0 And g_MouseIn = 1 Then 'Mouse Over but Not Pressed
        ...
        If g_MouseDown = 1 Then 'Mouse Over and Pressed
                If XPDefaultColors = True Then
...
Private Sub Draw3DEffect()
    ...
    ElseIf m_ButtonStyle = gbWinXP Then
            If g_MouseDown = 1 Then DrawWinXPButton 2  'BASILDI
            If g_MouseDown = 0 And g_MouseIn = 1 Then DrawWinXPButton 0, 1 'ÜSTÜNDE AMA BASILI DEÐÝL
            If g_MouseDown = 0 And g_MouseIn = 0 Then DrawWinXPButton 0 'DIÞARDA
    Else
        If g_MouseDown = 1 Then 'BASILDI
            If m_BEVEL < 2 Then
            ...
            End If
        End If
        If g_MouseDown = 0 And g_MouseIn = 1 Then 'ÜSTÜNDE AMA BASILI DEÐÝL
            If m_BEVEL < 2 Then
                DRAWRECT hdc, 0, 0, ScaleWidth, ScaleHeight, &H80000010
                DRAWRECT hdc, 0, 0, ScaleWidth + 1, ScaleHeight + 1, &H80000014
            Else
        ...
        If g_MouseDown = 0 And g_MouseIn = 0 And m_ButtonStyle = gbStandard Then 'OUT & STANDARD
            If m_BEVEL < 2 Then
                DRAWRECT hdc, 0, 0, ScaleWidth, ScaleHeight, &H80000010
                DRAWRECT hdc, 0, 0, ScaleWidth + 1, ScaleHeight + 1, &H80000014
            Else
...
Private Sub OverTimer_Timer()
    ...
    If hwnd <> WindowFromPoint(P.X, P.Y) Then
        ...
        If g_MouseDown = 1 Then  'Resfresh state
            g_MouseDown = 0
            Refresh
            g_MouseDown = 1
        End If
...
Private Function PlayASound(SoundFile As String) As Byte
    'ESKÝ HALÝ(ORJÝNAL)
'    bSuccess = PlaySound(SoundFile, vbNull, &H20000 _
'    + &H0 + &H10 + &H2)
'    PlayASound = bSuccess
...
If Enabled Then
    ...
    If None_Press_Disabled = 0 Then
             ...
             If g_HasFocus = 1 Then 'DRAW FOCUS HIGHLIGHT
                 DRAWRECT hdc, 1, 2, Gen - 1, Yuk - 2, KolorPressed '140,174,231
                 DrawLine 2, Yuk - 2, Gen - 2, Yuk - 2, COLOR_DarkenLightenColor(COLOR_UniColor(KolorPressed), -33) '107,130,239
                 DrawLine 2, 1, Gen - 2, 1, COLOR_DarkenLightenColor(COLOR_UniColor(KolorPressed), 65) '206,231,255
                 DrawLine 1, 2, Gen - 1, 2, COLOR_DarkenLightenColor(COLOR_UniColor(KolorPressed), 50) '189,215,247
                 DrawLine 2, 3, 2, Yuk - 3, COLOR_DarkenLightenColor(COLOR_UniColor(KolorPressed), 31) '181,209,240
                 DrawLine Gen - 3, 3, Gen - 3, Yuk - 3, COLOR_DarkenLightenColor(COLOR_UniColor(KolorPressed), 31) '181,209,240
             Else
             ...
             End If
             If HOVERING = 1 Then
                 DRAWRECT hdc, 1, 2, Gen - 1, Yuk - 2, KolorHover '255,178,49
                 DrawLine 2, Yuk - 2, Gen - 2, Yuk - 2, COLOR_DarkenLightenColor(KolorHover, -40) '231,150,0
                 DrawLine 2, 1, Gen - 2, 1, COLOR_DarkenLightenColor(KolorHover, 90)   '255,243,206
                 DrawLine 1, 2, Gen - 1, 2, COLOR_DarkenLightenColor(KolorHover, 35) '255,219,140
                 DrawLine 2, 3, 2, Yuk - 3, COLOR_DarkenLightenColor(KolorHover, 20) '255,203,107
                 DrawLine Gen - 3, 3, Gen - 3, Yuk - 3, COLOR_DarkenLightenColor(KolorHover, 20) '255,203,107
             End If
...
Private Sub SetAccessKeys() 'This Sub is all by Gonchuki
